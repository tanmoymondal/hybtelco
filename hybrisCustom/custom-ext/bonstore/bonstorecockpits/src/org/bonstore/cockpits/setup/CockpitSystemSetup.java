 ...
 *  
 */
package org.bonstore.cockpits.setup;
...
import org.bonstore.cockpits.constants.BonstoreCockpitsConstants;
 ...
 */
@SystemSetup(extension = BonstoreCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(BonstoreCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(BonstoreCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						BonstoreCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/bonstorecockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
